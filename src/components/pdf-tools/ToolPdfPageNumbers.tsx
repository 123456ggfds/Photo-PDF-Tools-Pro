...
export function ToolPdfPageNumbers() {
  ...
  const run = async () => { if (!file) return; setProcessing(true); try { const doc = await PDFDocument.load(file); const font = await doc.embedFont(StandardFonts.Helvetica); doc.getPages().forEach((p, i) => p.drawText(String(i + 1), { x: p.getWidth() - 36, y: 18, size: 10, font, color: rgb(0.8,0.8,0.8) })); setBytes(await doc.save()); } finally { setProcessing(false); } };
