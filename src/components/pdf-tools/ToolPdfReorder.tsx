...
export function ToolPdfReorder() {
  ...
  const download = () => { if (!bytes) return; const url = URL.createObjectURL(new Blob([bytesToBlobPart(bytes)], { type: "application/pdf" })); const a = document.createElement("a"); a.href = url; a.download = "reordered.pdf"; a.click(); URL.revokeObjectURL(url); };
