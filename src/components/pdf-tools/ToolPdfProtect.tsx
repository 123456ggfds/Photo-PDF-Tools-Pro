...
export function ToolPdfProtect() {
  ...
  const download = () => { if (!bytes) return; const url = URL.createObjectURL(new Blob([bytesToBlobPart(bytes)], { type: "application/pdf" })); const a = document.createElement("a"); a.href = url; a.download = `protected-${file?.name ?? "output.pdf"}`; a.click(); URL.revokeObjectURL(url); };
