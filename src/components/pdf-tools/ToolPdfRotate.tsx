...
export function ToolPdfRotate() {
  ...
  const run = async () => { if (!file) return; setProcessing(true); try { const doc = await PDFDocument.load(file); doc.getPages().forEach(p => p.setRotation(degrees(angle))); setBytes(await doc.save()); } finally { setProcessing(false); } };
  const download = () => { if (!bytes) return; const url = URL.createObjectURL(new Blob([bytesToBlobPart(bytes)], { type: "application/pdf" })); const a = document.createElement("a"); a.href = url; a.download = "rotated.pdf"; a.click(); URL.revokeObjectURL(url); };
